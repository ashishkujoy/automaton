...
const nfaToDfa = require('../src/NfaToDfa').nfaToDfa;
const fs = require('fs');
...
const nfaTodfaTestCases = testData.filter((data) => data.type === 'nfa-to-dfa');
...
describe('Automaton', function() {
    ...
    describe('NfaToDfa',function () {
        nfaTodfaTestCases.forEach(function(nfaToDfaTestCase) {
            describe(nfaToDfaTestCase.name, function () {
                let dfa = nfaToDfa(Nfa.getInstance(nfaToDfaTestCase.tuple));

                nfaToDfaTestCase["pass-cases"].forEach(function (validInput) {
                    it(getValidInputTestStatement(validInput), function () {
                        assert.isTrue(dfa.doesAccept(validInput))
                    })
                });

                nfaToDfaTestCase['fail-cases'].forEach(function (invalidInput) {
                    it(getInValidInputTestStatement(invalidInput), function () {
                        assert.isFalse(dfa.doesAccept(invalidInput))
                    })
                });
